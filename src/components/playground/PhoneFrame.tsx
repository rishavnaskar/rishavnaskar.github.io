...
export default function PhoneFrame({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "relative mx-auto aspect-[9/19] w-full max-w-[250px] rounded-[2.4rem]",
    ...
    >
      ...
      {/* screen — pinned 10px inside the frame with `absolute inset`, NOT
          `height:100%`. WebKit/iOS doesn't resolve a percentage height against
          an aspect-ratio parent, so `h-full` let the screen fall back to its
          (taller) content height and hang out the frame's bottom. Absolute
          inset takes its size from the frame's real rendered box, which Safari
          computes correctly. Radius matches the frame's inner corner. */}
      <div className="absolute inset-[10px] overflow-hidden rounded-[calc(2.4rem-10px)] bg-white">
        {/* notch */}
